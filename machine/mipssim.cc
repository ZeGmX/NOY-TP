...
int Machine::OneInstruction(Instruction *instr)
{
  ...
  case OP_ADD:
    ...
    if (!((int_registers[(int)instr->rs] ^ int_registers[(int)instr->rt]) & SIGN_BIT) &&
  ...
  case OP_SB:
    if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 1,
                       int_registers[(int)instr->rt]))
      return 0;
    break;

  case OP_SH:
    if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 2,
                       int_registers[(int)instr->rt]))
      return 0;
    break;
  ...
  case OP_SW:
    if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 4,
                       int_registers[(int)instr->rt]))
      return 0;
    break;
  ...
  case OP_SDC1:
    if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 4,
                       float_registers[(int)instr->ft]))
      return 0;
    if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra + 4), 4,
